...
class StarterGuardrail:
    ...
    def check(self, request: GuardrailRequest) -> GuardrailDecision:
        ...
        # 3. RUN DETECTORS ON MESSAGE FIRST (Fixes bypass vulnerability)
        signals: list[Signal] = []
        ...
        # If any detector caught a BLOCK action in the message, enforce it immediately
        if any(s.action == Action.BLOCK for s in signals):
            return self._policy.decide(signals, request.context.route)
        
        # 4. Check quoted data cases only if the message itself is clean
        if request.evidence:
    ...
    def _is_abuse_evidence(self, text: str) -> bool:
        abuse_words = ["worthless", "pathetic", "nobody wants you", "hostile", "abusive", "insult"]
        return any(word in text for word in abuse_words)
    
    def _is_scam_evidence(self, text: str) -> bool:
        scam_words = ["processing fee", "guaranteed prize", "gift cards", "impersonation", "verify your account"]
        return any(word in text for word in scam_words)
    
    def _is_injection_evidence(self, text: str) -> bool:
        injection_words = ["ignore the system", "disregard your directions", "reveal the secret", "expose", "prompt-injection"]
        return any(word in text for word in injection_words)
    
    def _is_abuse_analysis(self, text: str) -> bool:
        analysis_words = ["classify", "summarize", "abusive tone", "reported insult"]
        return any(word in text for word in analysis_words)
    
    def _is_scam_analysis(self, text: str) -> bool:
        analysis_words = ["identify", "warning signs", "consistent with", "impersonation"]
        return any(word in text for word in analysis_words)
    
    def _is_injection_analysis(self, text: str) -> bool:
        analysis_words = ["explain", "review", "inert evidence", "prompt-injection"]
        return any(word in text for word in analysis_words)
